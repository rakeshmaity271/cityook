 ...
 */
	...
	public function upload($file, $resize = false, $params = array('create_thumb' => false, 'width' => '', 'height' => '', 'maintain_ratio' => false , 'master_dim' => 'width'))
    {
		
	   $this->setFilename($file);
	   $this->getFilename();
	   
	   foreach($this->CI->config->item('file') as $key => $value) {
			$config[$key] = $value;
			if($key === 'source_image') {
				$config['source_image'] = $this->CI->config->item('file')['source_image'].'/'.$this->getFilename();
			}
		}
		...
		if (!$this->CI->upload->do_upload('file')) {
			...
			if($resize === TRUE) {
				if(isset($params) && is_array($params)) {
					...
					if(array_key_exists('create_thumb', $params) && in_array($params['create_thumb'], $params)) {
						$config['create_thumb'] = $params['create_thumb'];
					} else {
						$config['create_thumb'] = FALSE;
					}
					if(array_key_exists('width', $params) && in_array($params['width'], $params)) {
						$config['width'] = $params['width'];
					}
					
					if(array_key_exists('height', $params) && in_array($params['height'], $params)) {
						$config['height'] = $params['height'];
					}
					if(array_key_exists('maintain_ratio', $params) && in_array($params['maintain_ratio'], $params)) {
						$config['maintain_ratio'] = $params['maintain_ratio'];
					} else {
						$config['maintain_ratio'] = FALSE;
					}
					if(array_key_exists('master_dim', $params) && in_array($params['master_dim'], $params)) {
						if($params['master_dim'] === TRUE) {
							$config['master_dim'] = 'width';
						}
						$config['master_dim'] = FALSE;
					}
					foreach($this->CI->config->item('file') as $key => $value) {
						...
						// if($key === 'upload_path') {
						// 	$config['upload_path'] = $this->CI->config->item('file')['thumb_path'].'/'.$this->getFilename();
						// }
					}
				} else {
					foreach($this->CI->config->item('file') as $key => $value) {
			...
			$data = array(
					...
					'file_name' =>base_url().'/uploads/img/'.$this->getFilename().'?dummy='.rand(0,1000),
